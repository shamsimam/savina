 ...
 */
object SucOverRelaxScalaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxScalaActorBenchmark)
  }

  private final class SucOverRelaxScalaActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[ScalaActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[ScalaActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends ScalaActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[ScalaActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[ScalaActor[AnyRef]](s)
  ...
  class SorActor(
                  ...
                  ny: Int,
                  omega: Double,
                  sorSource: ScalaActor[AnyRef],
                  peer: Boolean
                  ) extends ScalaActor[AnyRef] {
    ...
    private var sorActors: Array[ScalaActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[ScalaActor[AnyRef]](s * (s - partStart + 1))
