 ...
 */
object ProdConsHabaneroSelectorBenchmark {
  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ProdConsHabaneroSelectorBenchmark)
  }

  private final class ProdConsHabaneroSelectorBenchmark extends Benchmark {
    ...
    private class ManagerActor(bufferSize: Int, numProducers: Int, numConsumers: Int, numItemsPerProducer: Int)
      extends HabaneroSelector[AnyRef](MessageSource.values().length) {
        ...
        theMsg match {
          case dm: ProdConsBoundedBufferConfig.DataItemMessage =>
            ...
            if (pendingData.size >= adjustedBufferSize) {
              disable(MessageSource.PRODUCER)
            }
            enable(MessageSource.CONSUMER)
