 ...
 */
object SleepingBarberScalaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SleepingBarberScalaActorBenchmark)
  }

  private final class SleepingBarberScalaActorBenchmark extends Benchmark {
  ...
  private case class Enter(customer: ScalaActor[AnyRef], room: ScalaActor[AnyRef])

  private case class Returned(customer: ScalaActor[AnyRef])


  private class WaitingRoomActor(capacity: Int, barber: BarberActor) extends ScalaActor[AnyRef] {

    private val self = this
    private val waitingCustomers = new ListBuffer[ScalaActor[AnyRef]]()
  ...
  private class BarberActor extends ScalaActor[AnyRef] {
  ...
  private class CustomerFactoryActor(idGenerator: AtomicLong, haircuts: Int, room: WaitingRoomActor) extends ScalaActor[AnyRef] {
    ...
    private def sendCustomerToRoom(customer: ScalaActor[AnyRef]) {
      val enterMessage = new Enter(customer, room)
      room.send(enterMessage)
    }
  }

  private class CustomerActor(val id: Long, factoryActor: CustomerFactoryActor) extends ScalaActor[AnyRef] {
