 ...
 */
object PingPongGparsActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new PingPongGparsActorBenchmark)
  }

  private final class PingPongGparsActorBenchmark extends Benchmark {
  ...
  private class PingActor(count: Int, pong: GparsActor[PingPongConfig.Message]) extends GparsActor[Message] {
  ...
  private class PongActor extends GparsActor[Message] {
    private var pongCount: Int = 0

    override def process(msg: PingPongConfig.Message) {
      msg match {
        case message: PingPongConfig.SendPingMessage =>
          val sender = message.sender.asInstanceOf[GparsActor[PingPongConfig.Message]]
