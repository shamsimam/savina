 ...
 */
object ApspScalaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ApspScalaActorBenchmark)
  }

  private final class ApspScalaActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val blockActors = Array.tabulate[ScalaActor[AnyRef]](numBlocksInSingleDim, numBlocksInSingleDim) {
        (i, j) =>
      ...
      for (bi <- 0 until numBlocksInSingleDim) {
        for (bj <- 0 until numBlocksInSingleDim) {
          ...
          val neighbors = new ListBuffer[ScalaActor[AnyRef]]()
  ...
  private case class ApspNeighborMessage(neighbors: ListBuffer[ScalaActor[AnyRef]]) extends ApspMessage

  private class ApspFloydWarshallActor(myBlockId: Int, blockSize: Int, graphSize: Int, initGraphData: Array[Array[Long]]) extends ScalaActor[AnyRef] {
    ...
    private val neighbors = new ListBuffer[ScalaActor[AnyRef]]()
