 ...
 */
object CigaretteSmokerScalaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new CigaretteSmokerScalaActorBenchmark)
  }

  private final class CigaretteSmokerScalaActorBenchmark extends Benchmark {
  ...
  private class ArbiterActor(numRounds: Int, numSmokers: Int) extends ScalaActor[AnyRef] {

    private val self = this
    private val smokerActors = Array.tabulate[ScalaActor[AnyRef]](numSmokers)(i => new SmokerActor(self))
  ...
  private class SmokerActor(arbiterActor: ArbiterActor) extends ScalaActor[AnyRef] {
    override def process(msg: AnyRef) {
      msg match {
