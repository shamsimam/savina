 ...
 */
object RadixSortFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new RadixSortFuncJavaActorBenchmark)
  }

  private final class RadixSortFuncJavaActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      var nextActor: FuncJavaActor[AnyRef] = validationActor
      while (radix > 0) {
  ...
  private class IntSourceActor(numValues: Int, maxValue: Long, seed: Long) extends FuncJavaActor[AnyRef] {
  ...
  private class SortActor(numValues: Int, radix: Long, nextActor: FuncJavaActor[AnyRef]) extends FuncJavaActor[AnyRef] {
