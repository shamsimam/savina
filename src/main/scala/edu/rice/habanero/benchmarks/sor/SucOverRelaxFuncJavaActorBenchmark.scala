 ...
 */
object SucOverRelaxFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxFuncJavaActorBenchmark)
  }

  private final class SucOverRelaxFuncJavaActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[FuncJavaActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[FuncJavaActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends FuncJavaActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[FuncJavaActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[FuncJavaActor[AnyRef]](s)
  ...
  class SorActor(
                  ...
                  ny: Int,
                  omega: Double,
                  sorSource: FuncJavaActor[AnyRef],
                  peer: Boolean
                  ) extends FuncJavaActor[AnyRef] {
    ...
    private var sorActors: Array[FuncJavaActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[FuncJavaActor[AnyRef]](s * (s - partStart + 1))
