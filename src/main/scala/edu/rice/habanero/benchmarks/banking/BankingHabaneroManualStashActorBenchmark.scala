 ...
 */
object BankingHabaneroManualStashActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BankingHabaneroManualStashActorBenchmark)
  }

  private final class BankingHabaneroManualStashActorBenchmark extends Benchmark {
  ...
  protected class Teller(numAccounts: Int, numBankings: Int) extends HabaneroActor[AnyRef] {
  ...
  protected class Account(id: Int, var balance: Double) extends HabaneroActor[AnyRef] {

    private var inReplyMode = false
    private var replyTeller: HabaneroActor[AnyRef] = null
      ...
      if (inReplyMode) {
        ...
        theMsg match {
          case dm: DebitMessage =>
            ...
            balance += dm.amount
            val creditor = dm.sender.asInstanceOf[HabaneroActor[AnyRef]]
            creditor.send(ReplyMessage.ONLY)

          case cm: CreditMessage =>

            balance -= cm.amount
            replyTeller = cm.sender.asInstanceOf[HabaneroActor[AnyRef]]
