 ...
 */
object BitonicSortHabaneroSelectorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BitonicSortHabaneroSelectorBenchmark)
  }

  private final class BitonicSortHabaneroSelectorBenchmark extends Benchmark {
  ...
  private case class NextActorMessage(actor: HabaneroActor[AnyRef])
  ...
  private class ValueDataAdapterActor(orderId: Int, nextActor: HabaneroSelector[AnyRef]) extends HabaneroActor[AnyRef] {
    override def process(msg: AnyRef) {
      msg match {
        case vm: ValueMessage =>

          nextActor.send(orderId, new DataMessage(orderId, vm.value))

        case dm: DataMessage =>

          nextActor.send(orderId, dm)

        case em: ExitMessage =>

          nextActor.send(nextActor.numMailboxes - 1, em)
          exit()
      }
    }
  }

  private class DataValueAdapterActor(nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
    override def process(msg: AnyRef) {
      msg match {
        case vm: ValueMessage =>
  ...
  private class RoundRobinSplitterActor(name: String, length: Int, receivers: Array[HabaneroActor[AnyRef]]) extends HabaneroActor[AnyRef] {
  ...
  private class RoundRobinJoinerActor(name: String, length: Int, numJoiners: Int, nextActor: HabaneroActor[AnyRef]) extends HabaneroSelector[AnyRef](numJoiners + 1) {
   ...
   */
  private class CompareExchangeActor(orderId: Int, sortDirection: Boolean, nextActor: HabaneroSelector[AnyRef]) extends HabaneroActor[AnyRef] {
      ...
      msg match {
        case vm: ValueMessage =>
          ...
          if (!valueAvailable) {
            ...
            if (sortDirection) {
              // UP sort
              nextActor.send(orderId, DataMessage(orderId, minK))
              nextActor.send(orderId, DataMessage(orderId, maxK))
            } else {
              // DOWN sort
              nextActor.send(orderId, DataMessage(orderId, maxK))
              nextActor.send(orderId, DataMessage(orderId, minK))
   ...
   */
  private class PartitionBitonicSequenceActor(orderId: Int, length: Int, sortDir: Boolean, nextActor: HabaneroSelector[AnyRef]) extends HabaneroActor[AnyRef] {
    ...
    val workerActors = Array.tabulate[HabaneroActor[AnyRef]](halfLength)(i => {
   ...
   */
  private class StepOfMergeActor(orderId: Int, length: Int, numSeqPartitions: Int, directionCounter: Int, nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
    ...
    val workerActors = Array.tabulate[HabaneroActor[AnyRef]](numSeqPartitions)(i => {
   ...
   */
  private class StepOfLastMergeActor(length: Int, numSeqPartitions: Int, sortDirection: Boolean, nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
   ...
   */
  private class MergeStageActor(P: Int, N: Int, nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {

    val forwardActor = {
      var loopActor: HabaneroActor[AnyRef] = nextActor
   ...
   */
  private class LastMergeStageActor(N: Int, sortDirection: Boolean, nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
   ...
   */
  private class BitonicSortKernelActor(N: Int, sortDirection: Boolean, nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
  ...
  private class ValidationActor(numValues: Int) extends HabaneroActor[AnyRef] {
