 ...
 */
object BankingFuncJavaManualStashActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BankingFuncJavaManualStashActorBenchmark)
  }

  private final class BankingFuncJavaManualStashActorBenchmark extends Benchmark {
  ...
  protected class Account(id: Int, var balance: Double) extends FuncJavaActor[AnyRef] {
    ...
    private var replyTeller: FuncJavaActor[AnyRef] = null
      ...
      if (inReplyMode) {
        ...
        theMsg match {
          case dm: DebitMessage =>
            ...
            balance += dm.amount
            val creditor = dm.sender.asInstanceOf[FuncJavaActor[AnyRef]]
            creditor.send(ReplyMessage.ONLY)

          case cm: CreditMessage =>

            balance -= cm.amount
            replyTeller = cm.sender.asInstanceOf[FuncJavaActor[AnyRef]]
