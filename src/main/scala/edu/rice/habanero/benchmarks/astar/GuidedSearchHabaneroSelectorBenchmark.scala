 ...
 */
object GuidedSearchHabaneroSelectorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new GuidedSearchHabaneroSelectorBenchmark)
  }

  private final class GuidedSearchHabaneroSelectorBenchmark extends Benchmark {
  ...
  private class Master extends HabaneroSelector[AnyRef](MessagePriority.values().length) {
    ...
    private def sendWork(workMessage: WorkMessage) {
      ...
      numWorkSent += 1
      workers(workerIndex).send(workMessage.priority, workMessage)
    }
  ...
  private class Worker(master: Master, id: Int) extends HabaneroSelector[AnyRef](MessagePriority.values().length) {
    ...
    override def process(theMsg: AnyRef) {
      theMsg match {
        case workMessage: WorkMessage =>
          search(workMessage)
          master.send(0, ReceivedMessage.ONLY)
        case _: StopMessage =>
          master.send(0, theMsg)
          exit()
        case _ =>
      }
    }
    ...
    private def search(workMessage: WorkMessage) {
        ...
        while (i < numNeighbors) {
          ...
          if (success) {
            if (loopNeighbor eq targetNode) {
              master.send(0, DoneMessage.ONLY)
              return
            } else {
      ...
      while (!workQueue.isEmpty) {
        ...
        master.send(newWorkMessage.priority, newWorkMessage)
      }
    }
  }
