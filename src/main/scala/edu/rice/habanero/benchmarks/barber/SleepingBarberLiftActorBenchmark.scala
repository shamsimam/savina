 ...
 */
object SleepingBarberLiftActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SleepingBarberLiftActorBenchmark)
  }

  private final class SleepingBarberLiftActorBenchmark extends Benchmark {
  ...
  private case class Enter(customer: LiftActor[AnyRef], room: LiftActor[AnyRef])

  private case class Returned(customer: LiftActor[AnyRef])


  private class WaitingRoomActor(capacity: Int, barber: BarberActor) extends LiftActor[AnyRef] {

    private val self = this
    private val waitingCustomers = new ListBuffer[LiftActor[AnyRef]]()
  ...
  private class CustomerFactoryActor(idGenerator: AtomicLong, haircuts: Int, room: WaitingRoomActor) extends LiftActor[AnyRef] {
    ...
    private def sendCustomerToRoom(customer: LiftActor[AnyRef]) {
      val enterMessage = new Enter(customer, room)
      room.send(enterMessage)
    }
  }

  private class CustomerActor(val id: Long, factoryActor: CustomerFactoryActor) extends LiftActor[AnyRef] {
