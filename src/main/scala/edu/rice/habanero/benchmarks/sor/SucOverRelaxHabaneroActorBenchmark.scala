 ...
 */
object SucOverRelaxHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxHabaneroActorBenchmark)
  }

  private final class SucOverRelaxHabaneroActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[HabaneroActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[HabaneroActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends HabaneroActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[HabaneroActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[HabaneroActor[AnyRef]](s)
  ...
  class SorActor(
                  ...
                  ny: Int,
                  omega: Double,
                  sorSource: HabaneroActor[AnyRef],
                  peer: Boolean
                  ) extends HabaneroActor[AnyRef] {
    ...
    private var sorActors: Array[HabaneroActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[HabaneroActor[AnyRef]](s * (s - partStart + 1))
