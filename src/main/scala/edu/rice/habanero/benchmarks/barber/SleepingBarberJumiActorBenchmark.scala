 ...
 */
object SleepingBarberJumiActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SleepingBarberJumiActorBenchmark)
  }

  private final class SleepingBarberJumiActorBenchmark extends Benchmark {
  ...
  private case class Enter(customer: JumiActor[AnyRef], room: JumiActor[AnyRef])

  private case class Returned(customer: JumiActor[AnyRef])


  private class WaitingRoomActor(capacity: Int, barber: BarberActor) extends JumiActor[AnyRef] {

    private val self = this
    private val waitingCustomers = new ListBuffer[JumiActor[AnyRef]]()
  ...
  private class BarberActor extends JumiActor[AnyRef] {
  ...
  private class CustomerFactoryActor(idGenerator: AtomicLong, haircuts: Int, room: WaitingRoomActor) extends JumiActor[AnyRef] {
    ...
    private def sendCustomerToRoom(customer: JumiActor[AnyRef]) {
      val enterMessage = new Enter(customer, room)
      room.send(enterMessage)
    }
  }

  private class CustomerActor(val id: Long, factoryActor: CustomerFactoryActor) extends JumiActor[AnyRef] {
