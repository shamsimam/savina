...
import edu.rice.habanero.benchmarks.recmatmul.MatMulConfig.{DoneMessage, MessagePriority, StopMessage}
 ...
 */
object MatMulHabaneroSelectorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new MatMulHabaneroSelectorBenchmark)
  }

  private final class MatMulHabaneroSelectorBenchmark extends Benchmark {
    ...
    def send(theMsg: AnyRef) {
      theMsg match {
        case workMessage: MatMulConfig.WorkMessage =>
          val channel = MessagePriority.values()(workMessage.priority)
          send(channel.ordinal(), theMsg)
        case _ =>
