 ...
 */
object ChameneosFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ChameneosFuncJavaActorBenchmark)
  }

  private final class ChameneosFuncJavaActorBenchmark extends Benchmark {
  ...
  private class ChameneosMallActor(var n: Int, numChameneos: Int) extends FuncJavaActor[ChameneosHelper.Message] {

    private final val self: FuncJavaActor[ChameneosHelper.Message] = this
    private var waitingChameneo: FuncJavaActor[ChameneosHelper.Message] = null
        ...
        case message: ChameneosHelper.MeetMsg =>
          if (n > 0) {
            if (waitingChameneo == null) {
              val sender = message.sender.asInstanceOf[FuncJavaActor[ChameneosHelper.Message]]
  ...
  private class ChameneosChameneoActor(mall: FuncJavaActor[ChameneosHelper.Message], var color: ChameneosHelper.Color, id: Int)
    extends FuncJavaActor[ChameneosHelper.Message] {
