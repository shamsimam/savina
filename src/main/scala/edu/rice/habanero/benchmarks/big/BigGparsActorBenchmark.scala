 ...
 */
object BigGparsActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BigGparsActorBenchmark)
  }

  private final class BigGparsActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val bigActors = Array.tabulate[GparsActor[AnyRef]](BigConfig.W)(i => {
  ...
  private case class NeighborMessage(neighbors: Array[GparsActor[AnyRef]]) extends Message

  private class BigActor(id: Int, numMessages: Int, sinkActor: GparsActor[AnyRef]) extends GparsActor[AnyRef] {
  ...
  private class SinkActor(numWorkers: Int) extends GparsActor[AnyRef] {
