 ...
 */
object ChameneosHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ChameneosHabaneroActorBenchmark)
  }

  private final class ChameneosHabaneroActorBenchmark extends Benchmark {
  ...
  private class ChameneosMallActor(var n: Int, numChameneos: Int) extends HabaneroActor[ChameneosHelper.Message] {

    private final val self: HabaneroActor[ChameneosHelper.Message] = this
    private var waitingChameneo: HabaneroActor[ChameneosHelper.Message] = null
        ...
        case message: ChameneosHelper.MeetMsg =>
          if (n > 0) {
            if (waitingChameneo == null) {
              val sender = message.sender.asInstanceOf[HabaneroActor[ChameneosHelper.Message]]
  ...
  private class ChameneosChameneoActor(mall: HabaneroActor[ChameneosHelper.Message], var color: ChameneosHelper.Color, id: Int)
    extends HabaneroActor[ChameneosHelper.Message] {
