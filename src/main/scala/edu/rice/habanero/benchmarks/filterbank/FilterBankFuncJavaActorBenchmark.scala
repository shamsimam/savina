 ...
 */
object FilterBankFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new FilterBankFuncJavaActorBenchmark)
  }

  private final class FilterBankFuncJavaActorBenchmark extends Benchmark {
  ...
  private class ProducerActor(numSimulations: Int) extends FuncJavaActor[FilterBankConfig.Message] {
    ...
    override def process(theMsg: FilterBankConfig.Message) {
      theMsg match {
        case message: FilterBankConfig.NextMessage =>
          val source = message.source.asInstanceOf[FuncJavaActor[FilterBankConfig.Message]]
  ...
  private abstract class FilterBankActor(nextActor: FuncJavaActor[FilterBankConfig.Message]) extends FuncJavaActor[FilterBankConfig.Message] {
  ...
  private class SourceActor(producer: FuncJavaActor[FilterBankConfig.Message], nextActor: FuncJavaActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {
  ...
  private class SinkActor(printRate: Int) extends FuncJavaActor[FilterBankConfig.Message] {
  ...
  private class BranchesActor(numChannels: Int, numColumns: Int, H: Array[Array[Double]], F: Array[Array[Double]], nextActor: IntegratorActor) extends FuncJavaActor[FilterBankConfig.Message] {
  ...
  private class BankActor(sourceId: Int, numColumns: Int, H: Array[Double], F: Array[Double], integrator: IntegratorActor) extends FuncJavaActor[FilterBankConfig.Message] {
  ...
  private class FirFilterActor(sourceId: String, peekLength: Int, coefficients: Array[Double], nextActor: FuncJavaActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {
  ...
  private class SampleFilterActor(sampleRate: Int, nextActor: FuncJavaActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {
  ...
  private class IntegratorActor(numChannels: Int, nextActor: FuncJavaActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {
  ...
  private class CombineActor(nextActor: FuncJavaActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {
