 ...
 */
object ApspJumiActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ApspJumiActorBenchmark)
  }

  private final class ApspJumiActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val blockActors = Array.tabulate[JumiActor[AnyRef]](numBlocksInSingleDim, numBlocksInSingleDim) {
        (i, j) =>
      ...
      for (bi <- 0 until numBlocksInSingleDim) {
        for (bj <- 0 until numBlocksInSingleDim) {
          ...
          val neighbors = new ListBuffer[JumiActor[AnyRef]]()
  ...
  private case class ApspNeighborMessage(neighbors: ListBuffer[JumiActor[AnyRef]]) extends ApspMessage

  private class ApspFloydWarshallActor(myBlockId: Int, blockSize: Int, graphSize: Int, initGraphData: Array[Array[Long]]) extends JumiActor[AnyRef] {
    ...
    private val neighbors = new ListBuffer[JumiActor[AnyRef]]()
