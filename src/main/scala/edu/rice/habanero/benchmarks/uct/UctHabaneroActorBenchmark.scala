 ...
 */
object UctHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new UctHabaneroActorBenchmark)
  }

  private final class UctHabaneroActorBenchmark extends Benchmark {
   ...
   * @author <a href="http://shams.web.rice.edu/">Shams Imam</a> (shams@rice.edu)
   */
  protected class RootActor extends HabaneroActor[AnyRef] {

    private final val ran: Random = new Random(2)
    private var height: Int = 1
    private var size: Int = 1
    private final val children = new Array[HabaneroActor[AnyRef]](UctConfig.BINOMIAL_PARAM)
    ...
    override def process(theMsg: AnyRef) {
      theMsg match {
        ...
        case booleanMessage: UctConfig.ShouldGenerateChildrenMessage =>
          val sender: HabaneroActor[AnyRef] = booleanMessage.sender.asInstanceOf[HabaneroActor[AnyRef]]
     ...
     */
    def checkGenerateChildrenRequest(childName: HabaneroActor[AnyRef], childHeight: Int) {
  ...
  protected object NodeActor {
    def createNodeActor(parent: HabaneroActor[AnyRef], root: HabaneroActor[AnyRef], height: Int, id: Int, comp: Int, urgent: Boolean): NodeActor = {
  ...
  protected class NodeActor(myParent: HabaneroActor[AnyRef], myRoot: HabaneroActor[AnyRef], myHeight: Int, myId: Int, myCompSize: Int, isUrgent: Boolean) extends HabaneroActor[AnyRef] {
