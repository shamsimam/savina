 ...
 */
object SortedListFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SortedListFuncJavaActorBenchmark)
  }

  private final class SortedListFuncJavaActorBenchmark extends Benchmark {
  ...
  private class Worker(master: Master, sortedList: SortedList, id: Int, numMessagesPerWorker: Int) extends FuncJavaActor[AnyRef] {
  ...
  private class SortedList extends FuncJavaActor[AnyRef] {
    ...
    override def process(msg: AnyRef) {
      msg match {
        ...
        case containsMessage: SortedListConfig.ContainsMessage =>
          ...
          val sender = containsMessage.sender.asInstanceOf[FuncJavaActor[AnyRef]]
