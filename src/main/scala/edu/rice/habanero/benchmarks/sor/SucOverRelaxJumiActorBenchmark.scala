 ...
 */
object SucOverRelaxJumiActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxJumiActorBenchmark)
  }

  private final class SucOverRelaxJumiActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[JumiActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[JumiActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends JumiActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[JumiActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[JumiActor[AnyRef]](s)
  ...
  class SorActor(
                  ...
                  ny: Int,
                  omega: Double,
                  sorSource: JumiActor[AnyRef],
                  peer: Boolean
                  ) extends JumiActor[AnyRef] {
    ...
    private var sorActors: Array[JumiActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[JumiActor[AnyRef]](s * (s - partStart + 1))
