 ...
 */
object ApspFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ApspFuncJavaActorBenchmark)
  }

  private final class ApspFuncJavaActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val blockActors = Array.tabulate[FuncJavaActor[AnyRef]](numBlocksInSingleDim, numBlocksInSingleDim) {
        (i, j) =>
      ...
      for (bi <- 0 until numBlocksInSingleDim) {
        for (bj <- 0 until numBlocksInSingleDim) {
          ...
          val neighbors = new ListBuffer[FuncJavaActor[AnyRef]]()
  ...
  private case class ApspNeighborMessage(neighbors: ListBuffer[FuncJavaActor[AnyRef]]) extends ApspMessage

  private class ApspFloydWarshallActor(myBlockId: Int, blockSize: Int, graphSize: Int, initGraphData: Array[Array[Long]]) extends FuncJavaActor[AnyRef] {
    ...
    private val neighbors = new ListBuffer[FuncJavaActor[AnyRef]]()
