 ...
 */
object SleepingBarberGparsActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SleepingBarberGparsActorBenchmark)
  }

  private final class SleepingBarberGparsActorBenchmark extends Benchmark {
  ...
  private case class Enter(customer: GparsActor[AnyRef], room: GparsActor[AnyRef])

  private case class Returned(customer: GparsActor[AnyRef])


  private class WaitingRoomActor(capacity: Int, barber: BarberActor) extends GparsActor[AnyRef] {

    private val self = this
    private val waitingCustomers = new ListBuffer[GparsActor[AnyRef]]()
  ...
  private class BarberActor extends GparsActor[AnyRef] {
  ...
  private class CustomerFactoryActor(idGenerator: AtomicLong, haircuts: Int, room: WaitingRoomActor) extends GparsActor[AnyRef] {
    ...
    private def sendCustomerToRoom(customer: GparsActor[AnyRef]) {
      val enterMessage = new Enter(customer, room)
      room.send(enterMessage)
    }
  }

  private class CustomerActor(val id: Long, factoryActor: CustomerFactoryActor) extends GparsActor[AnyRef] {
