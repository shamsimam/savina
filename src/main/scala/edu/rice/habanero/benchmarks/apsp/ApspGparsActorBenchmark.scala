 ...
 */
object ApspGparsActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ApspGparsActorBenchmark)
  }

  private final class ApspGparsActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val blockActors = Array.tabulate[GparsActor[AnyRef]](numBlocksInSingleDim, numBlocksInSingleDim) {
        (i, j) =>
      ...
      for (bi <- 0 until numBlocksInSingleDim) {
        for (bj <- 0 until numBlocksInSingleDim) {
          ...
          val neighbors = new ListBuffer[GparsActor[AnyRef]]()
  ...
  private case class ApspNeighborMessage(neighbors: ListBuffer[GparsActor[AnyRef]]) extends ApspMessage

  private class ApspFloydWarshallActor(myBlockId: Int, blockSize: Int, graphSize: Int, initGraphData: Array[Array[Long]]) extends GparsActor[AnyRef] {
    ...
    private val neighbors = new ListBuffer[GparsActor[AnyRef]]()
