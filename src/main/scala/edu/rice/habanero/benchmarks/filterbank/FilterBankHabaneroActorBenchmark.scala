 ...
 */
object FilterBankHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new FilterBankHabaneroActorBenchmark)
  }

  private final class FilterBankHabaneroActorBenchmark extends Benchmark {
  ...
  private class IntegratorActor(numChannels: Int, nextActor: HabaneroActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {

    private final val data = new util.ArrayList[util.Map[Integer, Double]]
    ...
    override def process(theMsg: FilterBankConfig.Message) {
      theMsg match {
        case message: FilterBankConfig.SourcedValueMessage =>
          ...
          var i: Int = 0
          while (i < dataSize) {
            val loopMap: util.Map[Integer, Double] = data.get(i)
          ...
          if (!processed) {
            val newMap = new util.HashMap[Integer, Double]
            newMap.put(sourceId, result)
            data.add(newMap)
          }
          val firstMap: util.Map[Integer, Double] = data.get(0)
  ...
  private class CombineActor(nextActor: HabaneroActor[FilterBankConfig.Message]) extends FilterBankActor(nextActor) {
