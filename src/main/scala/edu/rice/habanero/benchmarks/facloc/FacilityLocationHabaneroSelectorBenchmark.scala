 ...
 */
object FacilityLocationHabaneroSelectorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new FacilityLocationHabaneroSelectorBenchmark)
  }

  private final class FacilityLocationHabaneroSelectorBenchmark extends Benchmark {
  ...
  private case class CustomerMsg(producer: HabaneroSelector[AnyRef], point: Point) extends Msg
  ...
  private class ProducerActor(consumer: HabaneroSelector[AnyRef]) extends HabaneroSelector[AnyRef](1) {
    ...
    private def produceCustomer(): Unit = {
      consumer.send(0, CustomerMsg(selfActor, Point.random(FacilityLocationConfig.GRID_SIZE)))
      itemsProduced += 1
    }
      ...
      message match {
        case msg: NextCustomerMsg =>
          if (itemsProduced < FacilityLocationConfig.NUM_POINTS) {
            ...
            consumer.send(0, RequestExitMsg())
            exit()
          }
      }
    }
  ...
  private class QuadrantActor(parent: QuadrantActor,
                              ...
                              initCustomers: java.util.List[Point]) extends HabaneroSelector[AnyRef](1) {
      ...
      msg match {
        case customer: CustomerMsg =>
          ...
          if (children == null) {
            ...
            while (index <= 4) {
              ...
              if (loopChildBoundary.contains(point)) {
                children(index).send(0, customer)
                index = 5
              } else {
                index += 1
              }
          ...
          if (parent eq null) {
            ...
            customer.producer.send(0, NextCustomerMsg())
          }
        ...
        case facility: FacilityMsg =>
          ...
          if (fromChild) {
            ...
            children(siblingPos).send(0, FacilityMsg(Position.UNKNOWN, depth, point, false))
            ...
            if (children ne null) {
              children.foreach {
                loopChild =>
                  loopChild.send(0, FacilityMsg(Position.UNKNOWN, depth, point, false))
              }
            }
          }
    ...
    private def notifyParentOfFacility(p: Point, depth: Int): Unit = {
      ...
      if (parent ne null) {
        ...
        parent.send(0, FacilityMsg(positionRelativeToParent, depth, p, true))
      }
    }
    ...
    private def safelyExit(): Unit = {
      ...
      if (parent ne null) {
        ...
        parent.send(0, ConfirmExitMsg(numFacilities, numCustomers))
      } else {
