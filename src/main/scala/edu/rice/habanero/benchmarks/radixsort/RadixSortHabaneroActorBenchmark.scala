 ...
 */
object RadixSortHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new RadixSortHabaneroActorBenchmark)
  }

  private final class RadixSortHabaneroActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      finish(new HjSuspendable {
        override def run() = {
          ...
          var nextActor: HabaneroActor[AnyRef] = validationActor
          while (radix > 0) {
  ...
  private class IntSourceActor(numValues: Int, maxValue: Long, seed: Long) extends HabaneroActor[AnyRef] {
  ...
  private class SortActor(numValues: Int, radix: Long, nextActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
