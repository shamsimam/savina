 ...
 */
object PhilosopherFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new PhilosopherFuncJavaActorBenchmark)
  }

  private final class PhilosopherFuncJavaActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val philosophers = Array.tabulate[FuncJavaActor[AnyRef]](PhilosopherConfig.N)(i => {
  ...
  case class HungryMessage(philosopher: FuncJavaActor[AnyRef], philosopherId: Int)
  ...
  private class PhilosopherActor(id: Int, rounds: Int, counter: AtomicLong, arbitrator: ArbitratorActor) extends FuncJavaActor[AnyRef] {
  ...
  private class ArbitratorActor(numForks: Int) extends FuncJavaActor[AnyRef] {
