 ...
 */
object SucOverRelaxLiftActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxLiftActorBenchmark)
  }

  private final class SucOverRelaxLiftActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[LiftActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[LiftActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends LiftActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[LiftActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[LiftActor[AnyRef]](s)
  ...
  class SorActor(
                  ...
                  ny: Int,
                  omega: Double,
                  sorSource: LiftActor[AnyRef],
                  peer: Boolean
                  ) extends LiftActor[AnyRef] {
    ...
    private var sorActors: Array[LiftActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[LiftActor[AnyRef]](s * (s - partStart + 1))
