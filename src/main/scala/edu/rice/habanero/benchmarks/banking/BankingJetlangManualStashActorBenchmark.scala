 ...
 */
object BankingJetlangManualStashActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BankingJetlangManualStashActorBenchmark)
  }

  private final class BankingJetlangManualStashActorBenchmark extends Benchmark {
  ...
  protected class Teller(numAccounts: Int, numBankings: Int) extends JetlangActor[AnyRef] {
  ...
  protected class Account(id: Int, var balance: Double) extends JetlangActor[AnyRef] {

    private var inReplyMode = false
    private var replyTeller: JetlangActor[AnyRef] = null
      ...
      if (inReplyMode) {
        ...
        theMsg match {
          case dm: DebitMessage =>
            ...
            balance += dm.amount
            val creditor = dm.sender.asInstanceOf[JetlangActor[AnyRef]]
            creditor.send(ReplyMessage.ONLY)

          case cm: CreditMessage =>

            balance -= cm.amount
            replyTeller = cm.sender.asInstanceOf[JetlangActor[AnyRef]]
