 ...
 */
object UctFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new UctFuncJavaActorBenchmark)
  }

  private final class UctFuncJavaActorBenchmark extends Benchmark {
   ...
   * @author <a href="http://shams.web.rice.edu/">Shams Imam</a> (shams@rice.edu)
   */
  protected class RootActor extends FuncJavaActor[AnyRef] {

    private final val ran: Random = new Random(2)
    private var height: Int = 1
    private var size: Int = 1
    private final val children = new Array[FuncJavaActor[AnyRef]](UctConfig.BINOMIAL_PARAM)
    ...
    override def process(theMsg: AnyRef) {
      theMsg match {
        ...
        case booleanMessage: UctConfig.ShouldGenerateChildrenMessage =>
          val sender: FuncJavaActor[AnyRef] = booleanMessage.sender.asInstanceOf[FuncJavaActor[AnyRef]]
     ...
     */
    def checkGenerateChildrenRequest(childName: FuncJavaActor[AnyRef], childHeight: Int) {
  ...
  protected object NodeActor {
    def createNodeActor(parent: FuncJavaActor[AnyRef], root: FuncJavaActor[AnyRef], height: Int, id: Int, comp: Int, urgent: Boolean): NodeActor = {
  ...
  protected class NodeActor(myParent: FuncJavaActor[AnyRef], myRoot: FuncJavaActor[AnyRef], myHeight: Int, myId: Int, myCompSize: Int, isUrgent: Boolean) extends FuncJavaActor[AnyRef] {
