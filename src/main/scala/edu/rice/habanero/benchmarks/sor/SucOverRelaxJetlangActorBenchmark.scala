 ...
 */
object SucOverRelaxJetlangActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxJetlangActorBenchmark)
  }

  private final class SucOverRelaxJetlangActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[JetlangActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[JetlangActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends JetlangActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[JetlangActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[JetlangActor[AnyRef]](s)
  ...
  class SorActor(
    ...
    private var sorActors: Array[JetlangActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[JetlangActor[AnyRef]](s * (s - partStart + 1))
