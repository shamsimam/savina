...
import edu.rice.habanero.benchmarks.nqueenk.NQueensConfig.{DoneMessage, MessagePriority, ResultMessage, StopMessage}
 ...
 */
object NQueensHabaneroSelectorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new NQueensHabaneroSelectorBenchmark)
  }

  private final class NQueensHabaneroSelectorBenchmark extends Benchmark {
  ...
  private class Master(numWorkers: Int, priorities: Int) extends HabaneroSelector[AnyRef](MessagePriority.values().length) {
    ...
    private def sendWork(workMessage: NQueensConfig.WorkMessage) {
      workers(messageCounter).send(workMessage.priority, workMessage)
      messageCounter = (messageCounter + 1) % numWorkers
      numWorkSent += 1
    }

    def send(theMsg: AnyRef) {
      theMsg match {
        case workMessage: NQueensConfig.WorkMessage =>
          send(workMessage.priority, theMsg)
        case _ =>
          send(MessagePriority.P_LOWEST.ordinal(), theMsg)
      }
    }
    ...
    def requestWorkersToTerminate() {
      var i: Int = 0
      while (i < numWorkers) {
        workers(i).send(0, StopMessage.ONLY)
        i += 1
      }
    }
  }

  private class Worker(master: Master, id: Int) extends HabaneroSelector[AnyRef](MessagePriority.values().length) {
