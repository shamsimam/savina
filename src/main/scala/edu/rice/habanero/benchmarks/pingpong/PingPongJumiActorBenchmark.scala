 ...
 */
object PingPongJumiActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new PingPongJumiActorBenchmark)
  }

  private final class PingPongJumiActorBenchmark extends Benchmark {
  ...
  private class PingActor(count: Int, pong: JumiActor[PingPongConfig.Message]) extends JumiActor[Message] {
  ...
  private class PongActor extends JumiActor[Message] {
    private var pongCount: Int = 0

    override def process(msg: PingPongConfig.Message) {
      msg match {
        case message: PingPongConfig.SendPingMessage =>
          val sender = message.sender.asInstanceOf[JumiActor[PingPongConfig.Message]]
