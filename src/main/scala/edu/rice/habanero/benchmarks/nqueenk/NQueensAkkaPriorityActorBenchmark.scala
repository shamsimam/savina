 ...
 */
object NQueensAkkaPriorityActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new NQueensAkkaPriorityActorBenchmark)
  }

  private final class NQueensAkkaPriorityActorBenchmark extends Benchmark {
    def initialize(args: Array[String]) {
      AkkaActorState.setPriorityMailboxType(classOf[NQueensAkkaPriorityMailbox].getCanonicalName)
    ...
    def runIteration() {
      ...
      master(0) = system.actorOf(Props(new Master(numWorkers, priorities)).withDispatcher("akka.actor.prio-dispatcher"))
...
class NQueensAkkaPriorityMailbox(settings: ActorSystem.Settings, config: Config)
