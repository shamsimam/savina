 ...
 */
object PingPongFuncJavaActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new PingPongFuncJavaActorBenchmark)
  }

  private final class PingPongFuncJavaActorBenchmark extends Benchmark {
  ...
  private class PingActor(count: Int, pong: FuncJavaActor[PingPongConfig.Message]) extends FuncJavaActor[Message] {
  ...
  private class PongActor extends FuncJavaActor[Message] {
    private var pongCount: Int = 0

    override def process(msg: PingPongConfig.Message) {
      msg match {
        case message: PingPongConfig.SendPingMessage =>
          val sender = message.sender.asInstanceOf[FuncJavaActor[PingPongConfig.Message]]
