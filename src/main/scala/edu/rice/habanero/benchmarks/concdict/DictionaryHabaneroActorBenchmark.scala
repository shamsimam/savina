 ...
 */
object DictionaryHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new DictionaryHabaneroActorBenchmark)
  }

  private final class DictionaryHabaneroActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      finish(new HjSuspendable {
        override def run() = {
          val numWorkers = DictionaryConfig.NUM_ENTITIES
          val numMessagesPerWorker = DictionaryConfig.NUM_MSGS_PER_WORKER
  ...
  private class Dictionary(initialState: util.Map[Integer, Integer]) extends HabaneroActor[AnyRef] {
