 ...
 */
object ChameneosGparsActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ChameneosGparsActorBenchmark)
  }

  private final class ChameneosGparsActorBenchmark extends Benchmark {
  ...
  private class ChameneosMallActor(var n: Int, numChameneos: Int) extends GparsActor[ChameneosHelper.Message] {

    private final val self: GparsActor[ChameneosHelper.Message] = this
    private var waitingChameneo: GparsActor[ChameneosHelper.Message] = null
        ...
        case message: ChameneosHelper.MeetMsg =>
          if (n > 0) {
            if (waitingChameneo == null) {
              val sender = message.sender.asInstanceOf[GparsActor[ChameneosHelper.Message]]
  ...
  private class ChameneosChameneoActor(mall: GparsActor[ChameneosHelper.Message], var color: ChameneosHelper.Color, id: Int)
    extends GparsActor[ChameneosHelper.Message] {
