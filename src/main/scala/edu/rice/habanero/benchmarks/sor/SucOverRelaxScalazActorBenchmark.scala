 ...
 */
object SucOverRelaxScalazActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SucOverRelaxScalazActorBenchmark)
  }

  private final class SucOverRelaxScalazActorBenchmark extends Benchmark {
  ...
  case class SorBorder(borderActors: Array[ScalazActor[AnyRef]])

  case class SorBorderMessage(mBorder: SorBorder)

  case class SorStartMessage(mi: Int, mActors: Array[ScalazActor[AnyRef]])
  ...
  private class SorRunner(n: Int) extends ScalazActor[AnyRef] {

    private val s = SucOverRelaxConfig.DATA_SIZES(n)
    private val part = s / 2
    private val sorActors = Array.ofDim[ScalazActor[AnyRef]](s * (part + 1))

    private def boot(): Unit = {

      val myBorder = Array.ofDim[ScalazActor[AnyRef]](s)
  ...
  class SorActor(
    ...
    private var sorActors: Array[ScalazActor[AnyRef]] = null
  ...
  class SorPeer(
    ...
    private val sorActors = Array.ofDim[ScalazActor[AnyRef]](s * (s - partStart + 1))
