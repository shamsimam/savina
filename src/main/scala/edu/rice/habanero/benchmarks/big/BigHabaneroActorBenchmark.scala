 ...
 */
object BigHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BigHabaneroActorBenchmark)
  }

  private final class BigHabaneroActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      finish(new HjSuspendable {
        override def run() = {
          ...
          val bigActors = Array.tabulate[HabaneroActor[AnyRef]](BigConfig.W)(i => {
  ...
  private case class NeighborMessage(neighbors: Array[HabaneroActor[AnyRef]]) extends Message

  private class BigActor(id: Int, numMessages: Int, sinkActor: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {

    private var numPings = 0
    private var expPinger = -1
    private val random = new PseudoRandom(id)
    private var neighbors: Array[HabaneroActor[AnyRef]] = null
  ...
  private class SinkActor(numWorkers: Int) extends HabaneroActor[AnyRef] {
