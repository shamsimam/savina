 ...
 */
object SortedListHabaneroRWAbstractBenchmark {

  abstract class SortedListHabaneroRWAbstractBenchmark extends Benchmark {
  ...
  class Master(numWorkers: Int, numMessagesPerWorker: Int, policy: ReaderWriterPolicy) extends HabaneroActor[AnyRef] {
    ...
    private final val sortedList = new SortedList(policy)
    ...
    override def process(msg: AnyRef) {
      if (msg.isInstanceOf[SortedListConfig.EndWorkMessage]) {
        ...
        if (numWorkersTerminated == numWorkers) {
          sortedList.sendWriteMessage(EndWorkMessage.ONLY)
          exit()
        }
      }
    }
    ...
    override def process(msg: AnyRef) {
      ...
      if (messageCount <= numMessagesPerWorker) {
        ...
        if (anInt < sizePercent) {
          sortedList.sendReadMessage(new SortedListConfig.SizeMessage(this))
        } else if (anInt < (sizePercent + writePercent)) {
          sortedList.sendWriteMessage(new SortedListConfig.WriteMessage(this, random.nextInt))
        } else {
          sortedList.sendReadMessage(new SortedListConfig.ContainsMessage(this, random.nextInt))
        }
      } else {
  ...
  private class SortedList(policy: ReaderWriterPolicy) extends ReaderWriterActor[AnyRef](policy) {
