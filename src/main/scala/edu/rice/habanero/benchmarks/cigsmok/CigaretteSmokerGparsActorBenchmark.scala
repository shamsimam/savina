 ...
 */
object CigaretteSmokerGparsActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new CigaretteSmokerGparsActorBenchmark)
  }

  private final class CigaretteSmokerGparsActorBenchmark extends Benchmark {
  ...
  private class ArbiterActor(numRounds: Int, numSmokers: Int) extends GparsActor[AnyRef] {

    private val self = this
    private val smokerActors = Array.tabulate[GparsActor[AnyRef]](numSmokers)(i => new SmokerActor(self))
  ...
  private class SmokerActor(arbiterActor: ArbiterActor) extends GparsActor[AnyRef] {
    override def process(msg: AnyRef) {
      msg match {
