 ...
 */
object ApspJetlangActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new ApspJetlangActorBenchmark)
  }

  private final class ApspJetlangActorBenchmark extends Benchmark {
    ...
    def runIteration() {
      ...
      val blockActors = Array.tabulate[JetlangActor[AnyRef]](numBlocksInSingleDim, numBlocksInSingleDim) {
        (i, j) =>
      ...
      for (bi <- 0 until numBlocksInSingleDim) {
        for (bj <- 0 until numBlocksInSingleDim) {
          ...
          val neighbors = new ListBuffer[JetlangActor[AnyRef]]()
  ...
  private case class ApspNeighborMessage(neighbors: ListBuffer[JetlangActor[AnyRef]]) extends ApspMessage

  private class ApspFloydWarshallActor(myBlockId: Int, blockSize: Int, graphSize: Int, initGraphData: Array[Array[Long]]) extends JetlangActor[AnyRef] {
    ...
    private val neighbors = new ListBuffer[JetlangActor[AnyRef]]()
