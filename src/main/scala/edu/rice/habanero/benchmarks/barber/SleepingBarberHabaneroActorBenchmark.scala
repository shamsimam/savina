 ...
 */
object SleepingBarberHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SleepingBarberHabaneroActorBenchmark)
  }

  private final class SleepingBarberHabaneroActorBenchmark extends Benchmark {
  ...
  private case class Enter(customer: HabaneroActor[AnyRef], room: HabaneroActor[AnyRef])

  private case class Returned(customer: HabaneroActor[AnyRef])


  private class WaitingRoomActor(capacity: Int, barber: BarberActor) extends HabaneroActor[AnyRef] {

    private val self = this
    private val waitingCustomers = new ListBuffer[HabaneroActor[AnyRef]]()
  ...
  private class BarberActor extends HabaneroActor[AnyRef] {
  ...
  private class CustomerFactoryActor(idGenerator: AtomicLong, haircuts: Int, room: WaitingRoomActor) extends HabaneroActor[AnyRef] {
    ...
    private def sendCustomerToRoom(customer: HabaneroActor[AnyRef]) {
      val enterMessage = new Enter(customer, room)
      room.send(enterMessage)
    }
  }

  private class CustomerActor(val id: Long, factoryActor: CustomerFactoryActor) extends HabaneroActor[AnyRef] {
