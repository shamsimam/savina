 ...
 */
object CountingHabaneroActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new CountingHabaneroActorBenchmark)
  }

  private final class CountingHabaneroActorBenchmark extends Benchmark {
  ...
  private case class RetrieveMessage(sender: HabaneroActor[AnyRef])

  private case class ResultMessage(result: Int)

  private class ProducerActor(counter: HabaneroActor[AnyRef]) extends HabaneroActor[AnyRef] {
  ...
  private class CountingActor extends HabaneroActor[AnyRef] {
