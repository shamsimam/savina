 ...
 */
object SieveHabaneroSelectorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new SieveHabaneroSelectorBenchmark)
  }

  private final class SieveHabaneroSelectorBenchmark extends Benchmark {
    ...
    def runIteration() {
      finish(new HjSuspendable {
        override def run() = {
          ...
          producerActor.send(0, filterActor)
        }
      })
    }
  ...
  private class NumberProducerActor(limit: Long) extends HabaneroSelector[AnyRef](1) {
    override def process(msg: AnyRef) {
      msg match {
        case filterActor: PrimeFilterActor =>
          var candidate: Long = 3
          while (candidate < limit) {
            filterActor.send(0, LongBox(candidate))
            candidate += 2
          }
          filterActor.send(0, "EXIT")
          exit()
      }
    }
  }

  private class PrimeFilterActor(val id: Int, val myInitialPrime: Long, numMaxLocalPrimes: Int) extends HabaneroSelector[AnyRef](1) {
          ...
          case candidate: LongBox =>
            ...
            if (locallyPrime) {
              if (nextFilterActor != null) {
                ...
                nextFilterActor.send(0, candidate)
              } else {
                // Found a new prime!
