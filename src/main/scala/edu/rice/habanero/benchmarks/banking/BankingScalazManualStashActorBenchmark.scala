 ...
 */
object BankingScalazManualStashActorBenchmark {

  def main(args: Array[String]) {
    BenchmarkRunner.runBenchmark(args, new BankingScalazManualStashActorBenchmark)
  }

  private final class BankingScalazManualStashActorBenchmark extends Benchmark {
  ...
  protected class Account(id: Int, var balance: Double) extends ScalazActor[AnyRef] {
    ...
    private var replyTeller: ScalazActor[AnyRef] = null
      ...
      if (inReplyMode) {
        ...
        theMsg match {
          case dm: DebitMessage =>
            ...
            balance += dm.amount
            val creditor = dm.sender.asInstanceOf[ScalazActor[AnyRef]]
            creditor.send(ReplyMessage.ONLY)

          case cm: CreditMessage =>

            balance -= cm.amount
            replyTeller = cm.sender.asInstanceOf[ScalazActor[AnyRef]]
